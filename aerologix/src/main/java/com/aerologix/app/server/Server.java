...
public class Server {
    ...
    public Response createBooking(BookingData bookingData) {
        try{
            ...
            if(notFound) {
                tx.commit();
                logger.error("Error when creating a new booking. At least one of the objects related to the booking does not exist.");
                return Response.status(Response.Status.UNAUTHORIZED).entity("Cannot create a new booking with data that does not exist in the database").build();
            } else {
