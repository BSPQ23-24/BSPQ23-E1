...
public class FlightService {
    ...
    public Response createFlight(FlightData flightData) {
    	try{
            ...
            if(notFound) {
                tx.commit();
                logger.error("Error when creating a new flight. At least one of the objects related to the flight does not exist.");
                return Response.status(Response.Status.UNAUTHORIZED).entity("Cannot create a new flight with data that does not exist in the database").build();
            } else {
    ...
    public Response modifyBooking(FlightData flightData) {
    	 try{
             ...
             logger.info("Get the current booking data for: {}", flightData.getIdFlight());
             Flight flight = null;
             try { 
                 flight = pm.getObjectById(Flight.class, flightData.getIdFlight());
             } catch(JDOObjectNotFoundException e) {
                 logger.info("Booking with id '{}' does not exist in the database.", flightData.getIdFlight());
             }
             ...
             if(flight != null) {
                 ...
                 tx.commit();
                 logger.info("User modified: {}", flight);
                 return Response.ok().build();
             } else {
